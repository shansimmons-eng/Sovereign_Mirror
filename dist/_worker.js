...
async function handleRequest(request) {
  ...
  // Only handle /active_state.json requests
  if (url.pathname.endsWith('/active_state.json')) {
    return handleActiveStateRequest()
  }
...
/**
 * Generate active state with time-varying bolt/grain values
 */
function handleActiveStateRequest() {
  const now = Date.now()
  ...
  // Base state (could come from KV or simulate values)
  const state = {
    metadata: {
