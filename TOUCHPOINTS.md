...
| 14 | `/api/pgate/engage` POST | React → Node server | HTTP JSON | Mode enum validation; `validateNumber()` finite check |
| 15 | `/api/veracity/calculate` POST | React → Node server | HTTP JSON | `validateNumber()` finite check on both fields |
| 16 | `/api/quorum/calculate` POST | React → Node server | HTTP JSON | `validateNumber()` finite check; `activeNodes >= 1` |
| 17 | `/api/atrophy/calculate` POST | React → Node server | HTTP JSON | `validateNumber()` finite check on both fields |
| 18 | `/api/kernel/version` GET | React → Node server | HTTP | Static version response |
| 19 | `/api/crypto/status` GET | React → Node server → subprocess | HTTP → JSON-RL | Cached 30s in `CryptoStatusPanel`; `getCryptoResult()` error guard |
| 20 | `/api/crypto/keypair` POST | React → Node server → subprocess | HTTP → JSON-RL | 4KB body limit at Node server; 128KB key limit at Rust; `getCryptoResult()` guard |
| 21 | `/api/crypto/sign` POST | React → Node server → subprocess | HTTP → JSON-RL | Same body/key/sig limits; `message`+`secret_key` required |
| 22 | `/api/crypto/verify` POST | React → Node server → subprocess | HTTP → JSON-RL | Same; `message`+`signature`+`public_key` required |
...
| 25 | **zkProof.ts keypair cache** | `keypairCache[algorithm]` | In-memory only, process lifetime; algorithms ≤ 8 entries in practice |
